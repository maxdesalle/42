...
/*   Updated: 2021/10/12 17:49:31 by maxdesall        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ClapTrap.hpp"

void			message_print(int option)
{
	switch (option)
	{
		case (CONSTRUCTOR):
...
void			ClapTrap::attack(std::string const &target)
{
	...
	{
		std::cout << "ClapTrap " << get_name() << " tries to attack " << target;
...
void			ClapTrap::takeDamage(unsigned int amount)
{
	...
	std::cout << "ClapTrap " << get_name() << " lost " << amount << " hitpoints";
	std::cout << std::endl;
	std::cout << get_name() << ": " << "HP - " << get_hitpoints();
	std::cout << " / EP - " << get_energy_points();
	std::cout << std::endl;
...
void			ClapTrap::beRepaired(unsigned int amount)
{
	...
	set_hitpoints(get_hitpoints() + amount);
	std::cout << "ClapTrap " << get_name() << " is repaired";
	std::cout << std::endl;
	std::cout << get_name() << ": " << "HP - " << get_hitpoints();
	std::cout << " / EP - " << get_energy_points();
	std::cout << std::endl;
